...
window.addEventListener('load', function () {
  ...
  btnCerrarSesion.addEventListener('click', function () {
    ...
    if (cerrarSesion) {
      //limpiamos el localstorage y redireccionamos a login
      localStorage.clear();
      location.replace('./index.html');
    }
  });
  ...
  function obtenerNombreUsuario() { // Requerimientos API para header
    const settings = {
      method: 'GET',
      headers: {
        authorization: token
    ...
    fetch(urlUsuario, settings)
      ...
      .then(data => {
        ...
        const nombreUsuario = document.querySelector('.user-info p'); // nodo
        nombreUsuario.innerText = data.firstName; // ingreso nombre en el documento
      })
...
// consultar
    ...
    fetch(urlTareas, settings)
      ...
      .then(tareas => {
        ...
        renderizarTareas(tareas); // se manda el listado a renderizar
        botonesCambioEstado();
        botonBorrarTarea();
      })
      .catch(error => console.log(error));
...
//madar al listado de tareas completas
      if (tarea.completed) {
        contador++;
        tareasTerminadas.innerHTML += `
          <li class="tarea">
        ...
        //mandar listado de tareas sin terminar
        tareasPendientes.innerHTML += `
          <li class="tarea">
